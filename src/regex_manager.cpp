...
RegexManager::load_config(libconfig::Setting& cfg)
{
   try
   {
     ...
     for(unsigned int i = 0; i < count; i++) {
       ...
       rated_banning_regexes.push_back(new RatedRegex(cur_id, cur_rule, new RE2((const char*)(banned_regexes_list[i]["regex"])), observation_interval * 1000, threshold /(double)(observation_interval* 1000)));
